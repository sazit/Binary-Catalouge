...
struct binaryTreeNode {
	...
	binaryTreeNode operator=(binaryTreeNode *a) {
		info = a->info;
		llink = a->llink;
		rlink = a->rlink;
		return *this;
...
class binaryTreeType {
public:
	binaryTreeNode<elemType>* getRoot() {
		return root;
	}
	...
	void inorderTraversalFile(ofstream& fout) const;
...
private:
	void _print(binaryTreeNode<elemType> *root, int level);
	...
	void inorder(binaryTreeNode<elemType> *p, ofstream& fout) const;
	//Function to do an inorder traversal of the binary
	//tree to which p points.
...
template<class elemType>
void binaryTreeType<elemType>::printTree(void) {
	_print(root, 0);
	return;
}

template<class elemType>
void binaryTreeType<elemType>::_print(binaryTreeNode<elemType> *root,
		int level) {
	int i;
	if (root) {
		_print(root->rlink, level + 1);
		cout << "Level: " << setw(3) << level;

		for (i = 0; i <= level; i++)
			cout << "....";
		cout << setw(3) << root->info->getId() << " " << root->info->getName()
				<< "\n";
		_print(root->llink, level + 1);
	}
...
template<class elemType>
void binaryTreeType<elemType>::inorderTraversalFile(ofstream& fout) const {
	inorder(root, fout);
}
...
template<class elemType>
void binaryTreeType<elemType>::inorder(binaryTreeNode<elemType> *p, ofstream& fout) const {
	if (p != NULL) {
		inorder(p->llink, fout);
		p->info->display(fout);
		inorder(p->rlink, fout);
	}
}
